...
public class NDFSM {
    ...
    public DFSM toDFSM() {
        ...
        Set<Transition> deltaTag = new HashSet<>();
        State sTag = getStateFromSet(eps(initialState));
        ...
        for (int i = 0; i < activeStates.size(); i++) {
            ...
            if (!computed) {
                for (Character c : alphabet) {
                    State newState = null;
                    Set<State> newStateAsSet = new HashSet<>();
                    for (State q : states) {
                        if (activeStates.get(i).encode().contains(q.encode())) {
                            newStateAsSet.add(q);
                        }
                    }
                    for (State q : newStateAsSet) {
                        Set<State> transitionsFromq = tr.at(q, c);
                        for (State p : transitionsFromq) {
                            Set<State> tempStatesAsSet = eps(p);
                            tempStatesAsSet.add(q);
                            newState = getStateFromSet(tempStatesAsSet);
                        }
                        if (newState != null) {
                            Transition newTransition = new Transition(activeStates.get(i), c, newState);
                            boolean transitionExists = false;
                            for (Transition dTag : deltaTag) {
                                if (dTag.fromState().equals(activeStates.get(i)) && dTag.symbol().equals(c)) {
        ...
        Set<State> kTag = new HashSet<>(activeStates);
        Set<State> aTag = getAcceptingStates(kTag);
        try {
            return new DFSM(kTag, alphabet, deltaTag, sTag, aTag);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public State getStateFromSet(Set<State> stateAsSet) {
        Integer new_state_id = getIDFromString(State.encodeStateSet(stateAsSet));
        if (new_state_id != null) {
            return new IdentifiedState(new_state_id);
        }
        return null;
    }

    public Set<State> getAcceptingStates(Set<State> activeStates) {
        Set<State> aTag = new HashSet<>();
        for (State q : activeStates) {
            for (State aState : acceptingStates) {
                if (q.encode().contains(aState.encode())) {
                    aTag.add(q);
                }
            }
        }
    ...
    public Integer getIDFromString(String id) {
        try {
            return Integer.parseInt(id.replaceAll("\\s+", ""));
    ...
    public Set<State> eps(State state) {
        ...
        int visitLength = 1;
        eps.add(state);
        for (State epsState : eps) {
            Set<State> tempSet = tr.at(epsState, Alphabet.EPSILON);
            for (State s : tempSet) {
                if (!eps.contains(s)) {
                    eps.add(s);
                }
            }
            if (visitLength == eps.size()) {
                break;
            }
            visitLength = eps.size();
        }
        return eps;
    }
}
